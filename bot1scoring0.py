...
def findMid():
    ...
    midpointX = opp + x2
    midpointY = y1
    client.publish("blanotiger/debug",payload="MidpointX: "+str(midpointX),qos=0,retain=False)
...
def driveF():
    ...
    if phase == 1:
		...
		parsed = json.loads(r.text)
		newparsed = parsed
            botX = float(parsed["Blue Team Data"]["Circle"]["Object Center"]["X"])
...
def on_message(client,userdata,message):
	...
	rotate.rotate(q)
        ...
        readIn(msg)
        print("("+str(botX)+", "+str(botY)+")("+str(ballX)+", "+str(ballY)+")")
        findMid()
...
print("Subscribing to topic 'blanotiger/robot1'...")
client.subscribe("blanotiger/robot1",0)
print("Subscribed!")
